...
<template>
    <div :class="isCarousel ? 'flex w-max min-w-full gap-4' : 'flex w-full snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain pb-3 sm:grid sm:grid-cols-2 sm:overflow-visible sm:pb-0 lg:grid-cols-3 lg:gap-5'">
        <article v-for="product in products" :key="product.id" :class="isCarousel ? 'w-[75vw] max-w-72.5 shrink-0 snap-start sm:w-70' : 'w-[82vw] max-w-sm shrink-0 snap-start sm:w-auto sm:max-w-none sm:min-w-0'" class="ferro-card-entrance group flex h-full flex-col rounded-2xl border border-ferro-carbon-100/50 bg-ferro-white p-5 shadow-sm transition-colors duration-300 hover:border-ferro-yellow-500/40">
